...
**Asymmetry:** SC does **not** forceIdle on `rx.lastErrorCode`. This is intentional — FC currently only sends `MSG_ERROR` as the P1-8 reaction-time warning, which is informational and does not require SC to abort. No current FC code path sends `MSG_ERROR` as a halt signal. If that changes in the future, the convention must be updated here and the SC loop must add a `lastErrorCode` check.
